...
class GeneralAssessmentScreen extends StatefulWidget {
  final String? patientId;

  const GeneralAssessmentScreen({super.key, this.patientId});

  @override
  State<GeneralAssessmentScreen> createState() =>
      _GeneralAssessmentScreenState();
}

class _GeneralAssessmentScreenState extends State<GeneralAssessmentScreen> {
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              ...
              // FORM HEADER
              const Text(
                'Patient Visit Form A - General Assessment',
                style: TextStyle(
                  fontSize: 20,
                  fontWeight: FontWeight.bold,
                  color: Colors.black,
              ...
              // DIET HISTORY - REQUIRED FIELD
              const Text(
                'Have you ever been on diet to lose weight? *',
                style: TextStyle(
                  fontSize: 16,
                  fontWeight: FontWeight.bold,
                  color: Colors.black,
                ...
                children: [
                  Expanded(
                    child: ListTile(
                      ...
                      leading: Radio<String>(
                        value: 'Yes',
                        groupValue: _dietHistory,
                        onChanged: (value) =>
                            setState(() => _dietHistory = value),
                      ),
                    ),
                  ),
                  Expanded(
  ...
  Future<void> _saveAssessment() async {
    ...
    try {
      final assessmentExists = await _databaseService
          .generalAssessmentExistsForDate(_currentPatientId!, _visitDate!);
    ...
    try {
      final assessment = GeneralAssessment(
        patientId: _currentPatientId!,
        visitDate: _visitDate!,
        generalHealth: _generalHealth!,
        hasBeenOnDiet: _dietHistory == 'Yes',
        comments: _commentsController.text.trim(),
      );

      final response =
          await _assessmentRepository.addGeneralAssessment(assessment);
